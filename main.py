...
def delete_messages(chat_id, user_msg_id):
    ...
    # 2. Видаляємо попереднє повідомлення бота, якщо воно є в пам'яті
...
def send_bot_message(message, text, markup=None, photo_path=None):
    """Універсальна функція відправки: сама чистить чат і запам'ятовує нове повідомлення"""
    ...
    try:
        ...
        # Якщо помилка (наприклад, немає фото), шлемо просто текст
...
def send_welcome(message):
    # При старті теж чистимо історію (опціонально)
    delete_messages(message.chat.id, message.message_id)
...
def show_contacts(message):
    text = (
        "*КОНТАКТИ АДМІНІСТРАЦІЇ ШКОЛИ ТНВК 15*\n\n"
        "📞 *+380 98 7654 321* — Директор\n"
        "_(Оксана Романівна)_\n\n"
        "📞 *+380 97 7654 321* — Заступник директора\n"
        "_(Мацьковська Ганна Петрівна)_\n\n"
        "➖➖➖➖➖➖➖➖➖➖\n"
        "*Адміністрація:*\n"
...
def show_location(message):
    ...
    # Створюємо кнопку-посилання (Inline), яка відкриє Google Maps
...
def profile_history(message):
    ...
    # Передаємо markup=profiles_menu(), щоб кнопки профілів не зникали
...
def unknown_content(message):
    ...
    # Можна відправити повідомлення про помилку, яке зникне (якщо ускладнити логіку),
    # або просто ігнорувати, щоб не псувати вигляд меню.
    # Тут я залишив варіант без відповіді, щоб меню завжди було чистим.

# Запуск
if __name__ == '__main__':
    print("Бот запущено з новим меню...")
